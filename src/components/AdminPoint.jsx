...
function AdminPoint(props) {
  ...
  async function getDataPointList() {
    setListPoint([<div className="loading loading_center" key="loading-3"/>]);
    const data = await getSimpleData('point', 0, null, {'sort[cityId]': 1});
    if (data.response) {
  ...
  async function callBackDelete(name, id) {
    const question =  window.confirm(`Удалить пункт ${name}: ${id}`);
    if (question) {
      await deleteSubject(login.data.access_token, id, 'point');
      setUpdate(id);
    };
  };

  async function buildPointList() {
    if (mainData) {
      ...
      for (let i in data) {
        ...
        let nameCity = '';
        if (item.cityId) {
          nameCity = item.cityId.name;
        }
        const tile = 
        <div className='admin-page-table__point' key={getId()}>
  ...
  useEffect(getDataPointList, [update]);
  useEffect(buildPointList, [ mainData ]);
      ...
      <div className='admin-page-table__wrapped admin-page-table__wrapped_car'>
        ...
        <div className='admin-page__main-window'>
          <div className='admin-page__main-window-content admin-page__main-window-content_no-footer'>
            <div className='admin-page-table__point admin-page-table__name'>
              <div className='admin-page-table__tile'>
                <b>Город</b>
              </div>
              <div className='admin-page-table__tile'>
...
export default connect(putStateToProps)(AdminPoint);
