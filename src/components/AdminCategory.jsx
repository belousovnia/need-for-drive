...
function AdminCategory(props) {
  ...
  async function getDataCategoryList() {
    setListCategory([<div className="loading loading_center" key="loading-3"/>]);
    const data = await getSimpleData('category');
    if (data.response) {
  ...
  async function callBackDelete(name, id) {
    const question =  window.confirm(`Удалить категорию ${name}: ${id}`);
    if (question) {
      await deleteSubject(login.data.access_token, id, 'category');
      setUpdate(id);
    };
  };
  ...
  async function buildCategoryList() {
    if (mainData) {
      ...
      for (let i in data) {
        const item = data[i];
        const tile = 
        <div className='admin-page-table__category' key={getId()}>
  ...
  useEffect(getDataCategoryList, [update]);
  useEffect(buildCategoryList, [ mainData ]);
        ...
        <div className='admin-page__main-window'>
          <div className='admin-page__main-window-content admin-page__main-window-content_no-footer'>
            <SimpleBar 
              ...
              autoHide = { false }
            >
              <div className='admin-page-table__category admin-page-table__name'>
                <div className='admin-page-table__tile'>
                  <b>Название</b> 
                </div>
                <div className='admin-page-table__tile'>
...
export default connect(putStateToProps)(AdminCategory);
