...
function AdminOrderStatus(props) {
  const { login } = props;
  ...
  async function getDataOrderStatusList() {
    setListOrderStatus([<div className="loading loading_center" key="loading-3"/>]);
    const data = await getSimpleData('orderStatus');
    if (data.response) {
  ...
  async function callBackDelete(name, id) {
    const question =  window.confirm(`Удалить статус заказа ${name}: ${id}`);
    if (question) {
      await deleteSubject(login.data.access_token, id, 'orderStatus');
      setUpdate(id);
    };
  };

  async function buildOrderStatusList() {
    if (mainData) {
      const data = mainData.data.data;
      const newListOrderStatus = [];
      for (let i in data) {
        const item = data[i];
        const tile = 
        <div className='admin-page-table__order-status' key={getId()}>
        ...
        newListOrderStatus.push(tile)
      };
      setListOrderStatus(newListOrderStatus);
    };
  };

  // ------------------------------------------------------

  useEffect(getDataOrderStatusList, [update]);
  useEffect(buildOrderStatusList, [ mainData ]);
...
export default connect(putStateToProps)(AdminOrderStatus);