...
function AdminRateType(props) {
  ...
  const [ listRateType, setListRateType ] = useState();
  ...
  async function getDataRateTypeList() {
    setListRateType([<div className="loading loading_center" key="loading-3"/>]);
    const data = await getSimpleData('rateType');
    if (data.response) {
  ...
  async function callBackDelete(name, id) {
    const question =  window.confirm(`Удалить тип тарифа ${name}: ${id}`);
    if (question) {
      await deleteSubject(login.data.access_token, id, 'rateType');
      setUpdate(id);
    };
  };

  async function buildRateTypeList() {
    if (mainData) {
      const data = mainData.data.data;
      const newListRateType = [];
      for (let i in data) {
        const item = data[i];
        const tile = 
        <div className='admin-page-table__rate-type' key={getId()}>
        ...
        newListRateType.push(tile)
      };
      setListRateType(newListRateType);
    };
  };

  // ------------------------------------------------------

  useEffect(getDataRateTypeList, [update]);
  useEffect(buildRateTypeList, [ mainData ]);
        ...
        <div className='admin-page__main-window'>
          <div className='admin-page__main-window-content admin-page__main-window-content_no-footer'>
            ...
            <SimpleBar 
              ...
              autoHide = { false }
            >
              <div className='admin-page-table__rate-type admin-page-table__name'>
                <div className='admin-page-table__tile'>
                  <b>Название</b> 
                </div>
                <div className='admin-page-table__tile'>
...
export default connect(putStateToProps)(AdminRateType);
